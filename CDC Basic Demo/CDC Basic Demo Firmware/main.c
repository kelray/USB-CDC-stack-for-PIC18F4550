...
/** V A R I A B L E S ********************************************************/
#pragma udata
unsigned char USB_In_Buffer[64];	//maximum buffer i can get
unsigned char USB_Out_Buffer[32] = {"v"};
unsigned char high = 0x00,low = 0x00;	//,buffer[64];
int adcbuff = 0x0000;
unsigned char x[10] = {0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x0C,0x0A,0x0B},y;
int i = 0,count = 0, toggle = 0;
int delay = 100;
unsigned int duty_cycle;
unsigned int step_input = 0;
...
{   
	...
	//Peripheral configuration is declared here
	
	//Example: configure A/D
    ...
    while(1)
    {
		...
		//Example: output 00000011 on port D
		//PORTD = 0b00000011;
			ProcessIO();        
    }//end while
}//end main
...
{   
	...
	//Example: Echoing string sent from PC - use Putty or TeraTerm
	//getsUSBUSART(USB_Out_Buffer,64);
	//strcpy(USB_In_Buffer,USB_Out_Buffer);
